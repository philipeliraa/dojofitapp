          ...
          @for (aula of getAulasForDate(day.date); track aula.id) {
            <div class="bg-white rounded-lg shadow-sm p-3 mb-2 flex items-center justify-between"
              ...
              <div>
                ...
                <p class="text-xs text-gray-500">{{ aula.horaInicio }} - {{ aula.horaFim }}</p>
              </div>
              <span class="text-xs text-gray-500">{{ aula.checkinsConfirmados }}/{{ aula.capacidadeMaxima }}</span>
            </div>
          }
...
export class ProfessorScheduleComponent implements OnInit {
  ...
  ngOnInit() { this.updateWeek(); }

  prevWeek() { this.weekStart.setDate(this.weekStart.getDate() - 7); this.updateWeek(); }
  nextWeek() { this.weekStart.setDate(this.weekStart.getDate() + 7); this.updateWeek(); }
  ...
  getAulasForDate(date: string): Aula[] { return this.aulas().filter(a => a.data === date); }
  private getMonday(d: Date): Date { const day = d.getDay(); return new Date(d.setDate(d.getDate() - day + (day === 0 ? -6 : 1))); }
